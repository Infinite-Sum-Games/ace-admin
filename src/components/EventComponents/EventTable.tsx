...
const EventsTable: React.FC<EventsTableProps> = ({ eventType }) => {
  ...
  const [selectedYear, setSelectedYear] = useRecoilState(yearFilterState); // Year filter state
  ...
  // Filter events by selected date and status
  ...
  return (
    <>
      ...
      <Table className="border-2 border-gray-800 rounded-2xl">
        <TableHeader>
          <TableRow className="h-20">
            <TableHead className="w-1/12 text-center py-4 text-gray text-lg">ID</TableHead>
            <TableHead className="w-1/12 text-center py-4 text-gray text-lg">Title</TableHead>
            <TableHead className="w-2/12 text-center py-4 text-gray text-lg">Venue</TableHead>
            <TableHead className="w-2/12 text-center py-4 text-gray text-lg">Cost</TableHead>
            <TableHead className="w-2/12 text-center py-4 text-gray text-lg">Start</TableHead>
            <TableHead className="w-2/12 text-center py-4 text-gray text-lg">Status</TableHead>
            <TableHead className="w-1/12 text-center py-4 text-gray text-lg">Actions</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {filteredEvents.length > 0 ? (
            filteredEvents.map((event, index) => (
              <TableRow key={event.id} className={index % 2 === 0 ? 'bg-accent' : ''}>
                <TableCell className="w-1/12 text-center py-8 text-m">{event.id}</TableCell>
                <TableCell className="w-2/12 text-center py-8 text-m">
                  <div className="font-medium">{event.title}</div>
                </TableCell>
                <TableCell className="w-2/12 text-center py-8 text-m">{event.venue}</TableCell>
                <TableCell className="w-2/12 text-center py-8 text-m">
                  <Badge
                ...
                <TableCell className="w-2/12 text-center py-8 text-m">{event.start}</TableCell>
                <TableCell className="w-2/12 text-center py-8 text-m">
                  <Badge
      ...
      {editEventId && <EditEventForm eventId={editEventId} onClose={() => setEditEventId(null)} />}
    </>
  );
};
